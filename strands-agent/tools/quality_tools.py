...
# Add to strands-agent/tools/quality_tools.py
...
async def get_issues_with_context(
    ...
    Returns:
        List of issues with file context
    """
        ...
        try:
            response = await client.get(
                ...
                params={
                    ...
                    "ps": limit,
                    "resolved": "false",
                    "additionalFields": "flows,sources"
                }
            )
            ...
            logger.error(f"Failed to get issues with context: {e}")
            return []

@tool
async def create_quality_batch_mr(
    file_changes: Dict[str, str],
    issue_summary: Dict[str, Any],
    project_id: str
) -> Dict[str, Any]:
    """Create MR with all quality fixes
    
    Args:
        file_changes: Dict of file paths to new content
        issue_summary: Summary of issues being fixed
        project_id: GitLab project ID
    
    Returns:
        MR creation result
    """
    from .gitlab_tools import create_merge_request
    
    # Generate MR description
    description = f"""## 🔧 SonarQube Quality Fixes

This MR addresses {issue_summary.get('total_issues', 0)} quality issues:

### Summary:
- 🐛 **Bugs**: {issue_summary.get('bugs', 0)}
- 🔒 **Vulnerabilities**: {issue_summary.get('vulnerabilities', 0)}
- 💩 **Code Smells**: {issue_summary.get('code_smells', 0)}
...
"""
    ...
    return await create_merge_request(
        title=f"Fix {issue_summary.get('total_issues', 0)} SonarQube issues",
        description=description,
        changes=file_changes,
        source_branch=f"quality-fixes/{int(time.time())}",
        target_branch="main",
        project_id=project_id
    )